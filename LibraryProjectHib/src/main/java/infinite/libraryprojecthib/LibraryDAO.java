...
public class LibraryDAO {
      ...
      public List<TranBook> showTranBook(String user) {
    	  sfact = SessionHelper.getConnection();
		  Session session = sfact.openSession();
		  Criteria cr = session.createCriteria(TranBook.class);
		  cr.add(Restrictions.eq("userName", user));
		  List<TranBook> lstBooks = cr.list();
		  return lstBooks;
	}
     
      public List<TransReturn> showTransReturns(String user){
    	  sfact = SessionHelper.getConnection();
		  Session session = sfact.openSession();
		  Criteria cr = session.createCriteria(TransReturn.class);
		  cr.add(Restrictions.eq("userName", user));
		  List<TransReturn> lstBList=cr.list();
		  return lstBList;
    	  
      }
      public String returnBook(String userName, int id) {
    	        sfact = SessionHelper.getConnection();
    		    Session session = sfact.openSession();
    		    Transaction trans = session.beginTransaction();
    			TranBook tb=new TranBook();
    			tb = search(id, userName );
    			
        		TransReturn tr= new TransReturn();
        		Date fdate=tb.getFromDate();
        		tr.setBookId(id);
			...
			return "Your Book "+id+" Return successfully";
      ...
      public String isseuBook(TranBook tranBook) {
  	    ...
  	    java.sql.Date sdate=new Date(date.getTime());
  	    tranBook.setFromDate(sdate);
